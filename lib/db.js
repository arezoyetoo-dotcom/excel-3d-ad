...
function ensureAdminUser() {
  const { salt, hash } = hashPassword('ploi1357');
  const existing = state.users.find(u => u.email && u.email.toLowerCase() === 'arezoyetoo@gmail.com');
  if (existing) {
    existing.name = 'Arezoyetoo (Lead Architect)';
